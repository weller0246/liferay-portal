	...
	<liferay-frontend:edit-form-body>
		...
		<liferay-frontend:fieldset-group>
			...
			>
				...
				<aui:field-wrapper cssClass="form-group lfr-input-text-container">
					<aui:input disabled="<%= ListUtil.isEmpty(accountEntryRelationshipObjectFields) || (objectDefinition.isApproved() && objectDefinition.isAccountEntryRestricted()) %>" label="" labelOff='<%= LanguageUtil.get(request, "inactive") %>' labelOn='<%= LanguageUtil.get(request, "active") %>' name="accountEntryRestricted" onChange='<%= "event.preventDefault(); " + liferayPortletResponse.getNamespace() + "handleAccountEntryRestrictionToggleChange();" %>' type="toggle-switch" value="<%= objectDefinition.isAccountEntryRestricted() %>" />
				</aui:field-wrapper>
				...
				<clay:row>
					<clay:col
						md="11"
					>
						<aui:select disabled="<%= ListUtil.isEmpty(accountEntryRelationshipObjectFields) || !objectDefinition.isAccountEntryRestricted() || (objectDefinition.isApproved() && objectDefinition.isAccountEntryRestricted()) %>" name="accountEntryRestrictedObjectFieldId" showEmptyOption="<%= false %>">
